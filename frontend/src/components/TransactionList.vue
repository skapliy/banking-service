<template>
  <div class="transaction-list">
    ...
    <p v-if="actionError" class="error action-error">{{ actionError }}</p> </div>
</template>
...
export default {
  ...
  methods: {
    ...
    formatCurrency(amount) {
      ...
      // Добавляем знак + для положительных чисел, кроме начального остатка
      const sign = numAmount > 0 ? '+' : ''; // Убрал проверку на коммент
      return `${sign}${new Intl.NumberFormat('ru-RU', { style: 'currency', currency: 'RUB' }).format(numAmount)}`;
    },
